...
async fn main() -> std::io::Result<()> {
    ...
    // Initialize database
    database::init_db(&config.database_url).expect("Failed to initialize database");
        ...
        App::new()
                ...
                web::scope("/api")
                    ...
                    .service(
                        web::scope("/logs")
                            .route(
                                "/medications",
                                web::get().to(handlers::get_all_medication_logs),
                            )
            ...
            .service(
                web::scope("/ws")
                    ...
                    .route(
                        "/medications",
                        web::get().to(websocket::medication_websocket),
                    )
