...
use crate::database;
use crate::models::{Medication, MedicationLog, VitalsLog, MedicationStatement, Observation};
...
pub async fn get_medications(
    ...
    let med_iter = stmt.query_map([], |row| {
        Ok(Medication {
            id: row.get(0)?,
            name: row.get(1)?,
    ...
    let medications: Vec<Medication> = med_iter
...
fn validate_medication_name(name: &str) -> Result<(), actix_web::Error> {
    if name.is_empty() {
        return Err(actix_web::error::ErrorBadRequest("Medication name cannot be empty"));
    }
    if name.len() > 100 {
        return Err(actix_web::error::ErrorBadRequest("Medication name too long (max 100 characters)"));
    }
    // Sanitize: remove potentially dangerous characters
    if name.contains('<') || name.contains('>') || name.contains('"') || name.contains('\'') {
        return Err(actix_web::error::ErrorBadRequest("Invalid characters in medication name"));
    }
    Ok(())
}
...
fn validate_schedule_time(time: &str) -> Result<(), actix_web::Error> {
    ...
    if parts.len() != 2 {
        return Err(actix_web::error::ErrorBadRequest("Invalid time format. Use HH:MM"));
    }
    if let (Ok(hour), Ok(minute)) = (parts[0].parse::<u8>(), parts[1].parse::<u8>()) {
        if hour > 23 || minute > 59 {
            return Err(actix_web::error::ErrorBadRequest("Invalid time. Hour must be 0-23, minute must be 0-59"));
        }
    } else {
        return Err(actix_web::error::ErrorBadRequest("Time must be numeric (HH:MM)"));
    }
    Ok(())
}
...
pub async fn get_medication_logs(
    ...
    let log_iter = stmt.query_map(params![med_id], |row| {
        Ok(MedicationLog {
            id: row.get(0)?,
    ...
    let logs: Vec<MedicationLog> = log_iter
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| {
            error!("Collection error: {}", e);
            actix_web::error::ErrorInternalServerError("Database error")
...
pub async fn get_all_medication_logs(
    config: web::Data<AppConfig>,
) -> Result<HttpResponse> {
    let conn = database::get_connection(&config.database_url)
        .map_err(|e| {
...
pub async fn get_vitals(
    ...
    let vitals_iter = stmt.query_map([], |row| {
        Ok(VitalsLog {
            id: row.get(0)?,
    ...
    let vitals: Vec<VitalsLog> = vitals_iter
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| {
            error!("Collection error: {}", e);
            actix_web::error::ErrorInternalServerError("Database error")
...
fn validate_vitals(temperature: Option<f64>, heart_rate: Option<i32>) -> Result<(), actix_web::Error> {
    if let Some(temp) = temperature {
        if temp < 20.0 || temp > 45.0 {
            return Err(actix_web::error::ErrorBadRequest("Temperature out of valid range (20-45°C)"));
        }
    }
    if let Some(hr) = heart_rate {
        if hr < 30 || hr > 250 {
            return Err(actix_web::error::ErrorBadRequest("Heart rate out of valid range (30-250 bpm)"));
        }
    }
    Ok(())
}
...
pub async fn create_vitals(
    ...
    if !valid_statuses.contains(&status.as_str()) {
        return Err(actix_web::error::ErrorBadRequest("Invalid status. Must be: normal, abnormal, or warning"));
    }
...
pub async fn get_all_vitals_logs(
    config: web::Data<AppConfig>,
) -> Result<HttpResponse> {
